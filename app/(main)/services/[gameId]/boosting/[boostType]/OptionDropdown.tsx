...
import { op2, options2Type } from "@/app/components/types/Types";
...
function OptionDropdown({ gameN, data, boostType }: mainProps) {
	const mainNameer = useAppSelector((state) => state.extraOptionSlice);
	const nameer = mainNameer.extraOptiontState;
	...
	useEffect(() => {
		nameer.map((item) => {
			dispatch(gameNTypeDefiendCheck({ gameName: gameN, boostType: boostType }));
			if (item.gameName === gameN) {
				if (item.gameOptions2!.length === 0) {
					dispatch(option2Changed({ game: gameN, type: boostType, items: initValue }));
				}
			}
		});
	});
	const handleItem = (item: string[]) => {
		...
		dispatch(option2Changed({ game: gameN, type: boostType, items: mainResult }));
	};
