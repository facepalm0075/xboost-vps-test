...
export default function RankSelect({
	...
	let setRmStars = false;
	const maxCurrent = data.length;
	const maxRank = data[data.length - 1].rankNums[data[data.length - 1].rankNums.length - 1].mmr;
	let maxCurrentC = 0;
	...
	return (
		<div className="gameType-base selectRank-main">
			<div style={{ backgroundColor: rankColor }} className="selectRank-BgColor"></div>
			...
			<div
				style={setRmStars ? { display: "none" } : {}}
				className="flex selectRank-num-main mt-6 justify-center"
			>
				{rankStars.map((item, key) => {
					if (item.content === "MMR") {
						return (
							<div key={key} className="flex justify-center mt-3 selectRank-input">
								<button>
									<span
										onClick={() => {
											if (rank > maxRank) {
