...
export const savePaypalTransaction = async (details: any, orderId:string, gatewayName:string) => {  
  const session = await getServerSession(authOptions);
	const dbitems = await prisma.buyerPaymentGatewayTransactions.create({
      ...
      gatewayFee:Number(details.purchase_units[0].payments.captures[0].seller_receivable_breakdown.paypal_fee.value),
